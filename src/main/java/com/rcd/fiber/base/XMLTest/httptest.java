...
public class httptest {
    public static void main(String[] args) {
        // http请求获取json字符串
        String str = getGetDateByUrl("http://localhost:8888/telemetry/findall");

        // Json字符串转换转化为list
        List<TelemetryDTO> list = JSON.parseObject(str, new TypeReference<List<TelemetryDTO>>() {
        });
        for (TelemetryDTO stu : list) {
            System.out.println(stu.getDetectedValue());
        }
    }
